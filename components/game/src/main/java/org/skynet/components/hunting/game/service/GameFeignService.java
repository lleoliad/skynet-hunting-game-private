...
public interface GameFeignService {
    ...
    Result<OpenChestBO> chestOpen(@ApiParam(name = "openChestQuery", value = "开宝箱参数", required = true) @RequestBody OpenChestQuery openChestQuery);

    @PostMapping(value = "chest/winChest")
    Result<SkynetObject> winChest(@ApiParam(name = "winChestQuery", value = "胜利宝箱参数", required = true) @RequestBody WinChestQuery winChestQuery);
